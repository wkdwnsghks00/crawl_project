...
def crawl_product_data(url):
    try:
        ...
        # 브랜드가 데이터베이스에 존재하는지 확인하고 없다면 추가
        check_brand_query = "SELECT id FROM brand WHERE name = %(brand)s"
        cursor.execute(check_brand_query, {'brand': brand})
        existing_brand = cursor.fetchone()

        if existing_brand:
            brand_id = existing_brand[0]  # 기존 브랜드가 존재하면 브랜드 ID 가져오기
        else:
            # 브랜드가 없는 경우에만 새로운 브랜드를 데이터베이스에 추가
            add_brand_query = """
                INSERT INTO brand (name)
                VALUES (%(brand)s)
            """
            cursor.execute(add_brand_query, {'brand': brand})
